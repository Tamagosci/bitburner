...
async function hashnet(ns) {
	...
	while (areAllNodesMaxed(ns) === false) {
		...
		while (nodesOwned < MAX_NODES && nextNodeCost <= funds) {
			...
			if (DEBUG_MODE) ns.print('Bought hashnet node ' + nodesOwned);
			else report(ns);
			nodesOwned++;
		}
		...
		for (const upgrade of allNodeUpgrades) {
			...
			if (upgrade.cost <= funds) {
				...
				funds -= upgrade.cost;
				if (DEBUG_MODE) ns.print(`Upgraded node ${upgrade.index} ${upgrade.stat} to ${ns.hacknet.getNodeStats(upgrade.index)[upgrade.stat]}`);
				else report(ns);
			}
		}
...
function report(ns) {
	...
	for (let i = 0; i < nodesOwned; i++) {
		...
		formattedHash = formatMoney(node.production, 3).padStart(6, ' ') + ' h/s';
