...
export async function main(ns) {
  ...
  while (true) {
    ...
    */
    ...
    await ns.sleep(TURN_SLEEP * (Math.random() + 0.5))
  }
}
...
function checkNewGame(ns, gameInfo) {
  if (gameInfo.type === "gameOver") {
    ...
    const hasPill = ns.singularity.getOwnedAugmentations(false).includes('The Red Pill')
    let opponentList = (hasPill) ? opponent2 : opponent
    let opponentListWantFavor = opponentList
      .filter(opponent => player.factions.includes(opponent))
      .filter(opponent => ns.singularity.getAugmentationsFromFaction(opponent)
        .some(augment => ownedAugments.includes(augment) === false))
    //% chance to prefer factions I want favor with
    if (Math.random() < 0.66 || opponentListWantFavor.length === 0)
      ns.go.resetBoardState(opponentList[Math.floor(Math.random() * opponentList.length)], 13)
    else
      ns.go.resetBoardState(opponentListWantFavor[Math.floor(Math.random() * opponentListWantFavor.length)], 13)
    turn = 0
    ns.clearLog()
    getStyle(ns)
  }
