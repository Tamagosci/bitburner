...
function reassign(ns) {
	...
	for (let i = 0; i < members.length; i++) {
		...
		//Get some vigilantes
		else if (gangInfo.wantedPenalty < MAX_WANTED_PENALTY && gangInfo.wantedLevel > 1 && i % 2 === 1)
...
function report(ns) {
	...
	//Footer
	const gangPower = formatMoney(gangInfo.power, 2 - Math.floor(Math.log10(gangInfo.power)) % 3 + (gangInfo.power < 1e3) ? 1 : 0).padStart(5, ' ');
