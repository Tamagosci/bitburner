...
const MAX_MONEY_DRIFT_PCT = 0.1;	// This is how far from 100% money we allow the server to be before growing (0.1 = 10%)
const DEFAULT_PCT = 0.25;			// This is the default percentage of money we want to hack from the server in a single pass (0.1 = 10%)
...
export async function main(ns) {
	// Parameters
	const [target, pct = DEFAULT_PCT, loop = true] = ns.args;
	await v1(ns, target, pct, loop);
}

/** 
 * @param {NS} ns 
 * @param {string} server
 * @param {number} pct Percent to hack each cycle
 * @param {boolean} loop
 */
export async function v1(ns, target, pct, loop) {
	ns.disableLog('ALL');
	ns.atExit(ns.closeTail);

	// If the user passes xp as a target, we grow joesguns for XP
	let xpMode = (target == 'xp');
	...
	await Exploit(ns, target, pct, xpMode, loop);
}
 ...
 * @param {string} server
 * @param {number} pct Percent to hack each cycle
 * @param {boolean} xpMode
 * @param {boolean} loop
 */
export async function Exploit(ns, server, pct, xpMode, loop = true) {
	...
	do {
		...
		if (!startedAnything) {
			...
			await ns.sleep(6000); // If we didn't have enough ram to start anything, we need to sleep here to avoid a lock
		}
	} while (loop);
}
...
/**
 * @param {NS} ns
 * @return {number} Percent of free ram
 */
export function MemoryReport(ns) {
