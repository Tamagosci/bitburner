import { formatMoney, getServerSymbols } from 'disease.js';
import { prime, weakenToMin } from 'primer.js';
import { Stonk } from 'drain.js';

const TRANSACTION_COST = 100_000;
const MIN_TRANSACTION = 50_000_000;
const BUDGET = 0.8;

const MAX_RAM_USAGE = 0.8;
const HACK_TARGET = 0.2;
const PAUSE_BEFORE_SELLING = 60e3;
...
export async function main(ns) {
	let [target, loop = true] = ns.args;
	await manipulate(ns, target, loop);
}
 ...
 * @param {boolean} loop
 */
async function manipulate(ns, target, loop) {
	//Logging
	ns.disableLog('ALL');
	ns.clearLog();
	ns.tail();
	...
	do {
		//Buy shorts
		if (SHORTS_AVAILABLE) buyShorts(ns, stonk);
		//Decrease forecast
		await down(ns, target);
		//Pause
		ns.printf('INFO Sleeping for %s seconds before selling shorts.', (PAUSE_BEFORE_SELLING / 1e3).toFixed(0));
		await ns.sleep(PAUSE_BEFORE_SELLING);
		//Sell shorts
		if (SHORTS_AVAILABLE) sellShorts(ns, stonk);
		...
		await up(ns, target);
		//Pause
		ns.printf('INFO Sleeping for %s seconds before selling longs.', (PAUSE_BEFORE_SELLING / 1e3).toFixed(0));
		await ns.sleep(PAUSE_BEFORE_SELLING);
		//Sell longs
		sellLongs(ns, stonk);
	} while (loop);
